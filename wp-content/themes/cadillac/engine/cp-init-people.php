...
function nvr_post_type_people() {
	...
	register_taxonomy('peoplecat', 'peoplepost',array('hierarchical' => true, 'label' =>  __('People Categories', THE_LANG ), 'singular_name' => __('Category', THE_LANG ))
	);
}
...
function nvr_people_manage_column($column_name){
	...
	switch($column_name){
		case 'images':
			...
			if($nvr_imagesrc){
				echo '<img src="'.$nvr_imagesrc[0].'" width="50" alt="'.$nvr_title.'" />';
			}else{
		...
		case 'peoplecat':
			...
			if($nvr_postterms){
				...
				foreach($nvr_postterms as $nvr_postterm){
					$nvr_termlists[] = '<a href="'.admin_url('edit.php?'.$nvr_taxonom.'='.$nvr_postterm->slug.'&post_type='.$nvr_posttype).'">'.$nvr_postterm->name.'</a>';
				}
		...
		foreach ($nvr_filters as $nvr_tax_slug) {
			...
			if(count($nvr_terms)){
				foreach ($nvr_terms as $nvr_term) { 
					...
					echo '<option value='. $nvr_term->slug. $nvr_selectedstr . '>' . $nvr_term->name .' (' . $nvr_term->count .')</option>'; 
				}
			}
			echo "</select>";
		}

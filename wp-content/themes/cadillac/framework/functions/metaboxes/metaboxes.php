				...
				foreach($sectionarrs as $sectionar){
					...
					$returnstring .= '<div class="nvr_sectionrow" id="nvr_sectionrow'.$field['id'].'_'.$sectionno.'">';
						...
						$returnstring .= '<table class="nvr_sectiontable" cellpadding="0" cellspacing="0" border="0">';
							$returnstring .= '<tr>';
								$returnstring .= '<td class="small"><label>'.__('BG Color', THE_LANG). '</label><br /><input type="text" name="'. $field['id']. '['.$i.'][backgroundcolor]" value="'.htmlspecialchars($sectionar['backgroundcolor']).'" /></td>';
								$returnstring .= '<td class="large"><label>'.__('BG Image', THE_LANG). '</label><br /><input type="text" name="'. $field['id']. '['.$i.'][background]" value="'.htmlspecialchars($sectionar['background']).'" /></td>';
								$returnstring .= '<td class="small"><label>'.__('Custom Class', THE_LANG). '</label><br /><input type="text" name="'. $field['id']. '['.$i.'][customclass]" value="'.htmlspecialchars($sectionar['customclass']).'" /></td>';
							$returnstring .= '</tr>';
							$returnstring .= '<tr>';
								$returnstring .= '<td colspan="3"><label>'.__('Content', THE_LANG). '</label><br /><textarea name="'. $field['id']. '['.$i.'][content]" rows="9">'. htmlspecialchars($sectionar['content']) .'</textarea></td>';
							$returnstring .= '</tr>';
						$returnstring .= '</table>';
					$returnstring .= '</div>';
					$i++;

...
?>
	...
	jQuery(document).ready(function(){
			...
			?>
			jQuery("input#buttonarray<?php echo $nvr_value['id']; ?>").click(function(evt){
				...
				?>
				var textarrayname = jQuery('input#<?php echo $nvr_value['id']; ?>').val();
				var textarrayid = jQuery.trim(textarrayname).toLowerCase().replace(" ","-");
				if(textarrayid!=""){
					jQuery.ajax({
						type : "POST",
						url : '<?php echo $nvr_url; ?>',
						data : "action=savearray&textarrayid=<?php echo $nvr_value['id']; ?>&arrayid="+textarrayid+"&<?php echo $nvr_value['id']; ?>="+textarrayname,
						success : function(data){
							var arraycage = 'div#<?php echo $nvr_value['id']."-cage"; ?>';
							var anchorarray = '<a href="'+textarrayid+'" rel="<?php echo $nvr_value['id']; ?>" id="'+textarrayid+'" class="delbutton">'+textarrayname+'</a>';
							jQuery(arraycage).append(anchorarray);
							jQuery(arraycage + ' a.delbutton').click(delbuttononclick);
							jQuery('input#<?php echo $nvr_value['id']; ?>').val('');
						}
					});
				}else{
		...
		<?php 
			foreach ($nvr_optionstheme as $nvr_value) {
				if ($nvr_value['type'] == "open") { 
					...
					echo '<a id="of-option-'. $nvr_valueopenname .'-tab" class="nav-tab nav-tab-active" title="'. $nvr_value['name'] .'" href="#of-option-'. $nvr_valueopenname .'">';
					echo $nvr_value['name'];
					echo '</a>';
				}
			}
	...
	<div class="bordertitle"></div>
		...
		?>
		 ...
		 <div id="<?php echo $nvr_valueopenname; ?>" class="tab_content" >
		...
		<tr valign="top"> 
		    ...
		    <td class="rowfield">
		        <input name="<?php echo $nvr_value['id']; ?>" size="60" id="<?php echo $nvr_value['id']; ?>" type="text" value="" /> 
                <input name="buttonarray<?php echo $nvr_value['id']; ?>" id="buttonarray<?php echo $nvr_value['id']; ?>" type="button" class="button-primary" value="<?php _e("Add", THE_LANG); ?>" /><br />
                <span class="setting-description"><?php _e('Please enter name of new sidebar', THE_LANG); ?></span><br />
 				<div id="<?php echo $nvr_value['id']; ?>-cage" class="delcage">
               	<?php 
					$nvr_textarrayval = get_option($nvr_value['id']);
					if(is_array($nvr_textarrayval)){
						foreach($nvr_textarrayval as $nvr_ids => $nvr_val){
							echo '<a href="'.$nvr_ids.'" rel="'.$nvr_value['id'].'" id="'.$nvr_ids.'" class="delbutton">'.$nvr_val.'</a>';
						}
					}
				?>
                <div class="clear"></div>

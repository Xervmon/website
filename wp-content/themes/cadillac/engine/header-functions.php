...
if(!function_exists("nvr_print_headmiscellaneous")){
	function nvr_print_headmiscellaneous(){
		...
		echo '<link rel="shortcut icon" href="' . $nvr_favicon_url . '" />';
...
?>
		<?php if($nvr_logotype == 'textlogo'){ ?>
			...
			<h1><a href="<?php echo home_url( '/'); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', THE_LANG ) ); ?>"><?php echo $nvr_sitename; ?></a></h1><span class="desc"><?php echo $nvr_tagline; ?></span>
        
        <?php } else { ?>
        	
            <div class="logoimg">
            <a href="<?php echo home_url( '/' ) ; ?>" title="<?php echo $nvr_sitename; ?>" >
                <img src="<?php echo $nvr_logoimage['url']; ?>" alt="<?php echo $nvr_sitename; ?>" class="darklogo" />
                <img src="<?php echo $nvr_logoimagelight['url']; ?>" alt="<?php echo $nvr_sitename; ?>" class="lightlogo" />
            </a>
            </div>
...
if(!function_exists("nvr_searchform")){
	function nvr_searchform($nvr_id="", $nvr_class=""){
		...
		$nvr_output = '<div class="'.$nvr_class.'">';
			$nvr_output .= '<form method="get" id="'.$nvr_id.'" class="btntoppanel" action="'. esc_url( home_url( '/' ) ) .'">';
				$nvr_output .= '<input type="submit" class="submit" name="submit" value="" />';
				$nvr_output .= '<div class="searcharea">';
					$nvr_output .= '<input type="text" name="s" class="txtsearch" placeholder="'. $nvr_searchtext .'" value="" />';
...
if(!function_exists("nvr_minicart")){
	function nvr_minicart($nvr_id="",$nvr_class=""){
		...
		$nvr_output = '<div id="'.$nvr_id.'" class="'.$nvr_class.'">';
			$nvr_output .= '<a class="topcartbutton btnpanel" href="'.$nvr_link.'"><i class="shopicon fa fa-shopping-cart"></i><span class="cart_subqty"><i class="cart_totalqty">'.$nvr_totalqty.' '.__('Items', THE_LANG).'</i><i class="cart_subtotal">'.$nvr_cart_subtotal.'</i></span></a>';
...
if(!function_exists('nvr_primary_menu')){
	function nvr_primary_menu($nvr_class=''){
		echo '<section class="navigation '.$nvr_class.'">';
...
if(!function_exists('nvr_output_headertext2')){
	function nvr_output_headertext2(){
		...
		if($nvr_headertext){
			echo '<div class="toptext text2"><span>'. do_shortcode($nvr_headertext) .'</span></div>';
		}
	}
